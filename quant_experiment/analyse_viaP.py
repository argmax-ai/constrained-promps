...
def main():
    folder = "output/experiments_viaP"
    df = pd.DataFrame()
    cut_failed = True
    savefiles = []
    ...
    for sf in savefiles:
        ...
        if hasattr(save_vipmp.env, "t_via_points"):
            n_viaP = len(save_vipmp.env.t_via_points)
        else:
            n_viaP = len(save_vipmp.env.obstacles)
        df = df.append(
            {
                "n_viaP": n_viaP,
    ...
    loss_hist = df
    print(
        "viaP\tn\tfail\t\tviolations\tkl\t\t\tviolations(S)\tkl(S)\t\truntime\t\truntime(S)"
    )
    for i in range(1, 4):
        df_o = df[df.n_viaP == i]
